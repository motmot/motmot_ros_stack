...
def set_frequency(freq, ignore_version_mismatch):
    ...
    dev.set_frames_per_second_approximate( freq )
    t_start = time.time()
    n_secs = 5.0
...
if __name__=='__main__':
    ...
    if options.freq is None:
        parser.error('No requency specified')
        sys.exit(1)

    set_frequency(options.freq, options.ignore_version_mismatch)
